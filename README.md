...
### 前台
- 📝 Markdown 文章渲染（Shiki 代码高亮 + KaTeX 数学公式）
- 🎬 电影感暗色主题 + 黄昏亮色模式 + Canvas 粒子背景
- 📊 学习时间线，按月分组展示
- 🔍 关键字搜索 + AI 语义搜索（DeepSeek V3，卷卷对话）
- 🏷️ 分类 & 标签独立浏览页，支持搜索筛选，防溢出只显示 Top10
- 📱 响应式适配手机/平板（汉堡菜单 / 后台侧边抽屉）
- 💬 Giscus 评论区（跟随博客主题切换暗/亮色）
- 🔗 文章前后篇导航 + 一键复制链接分享
- 📡 RSS 订阅 (`/api/feed.xml`)
- 👁️ 文章阅读量统计
- 🎯 自定义光标（圆点 + 延迟跟随环）
...
- 🐱 **桌宠「卷卷」** — Canvas 序列帧动画（idle 22帧 / walk 12帧 / chat 9帧），预加载防止切换闪现，自动随机漫步、拖拽移动、方向翻转

### 部署与优化
- 🌐 **自定义域名 + Cloudflare CDN** — `asher1211.blog`，国内无需翻墙
- 📈 **SEO 优化** — sitemap.xml / robots.txt / OpenGraph 标签
- ⚡ **ISR 缓存** — 前台页面每 60 秒增量更新
- 📦 **数据库精简** — 移除了未使用的 pgvector 向量字段
